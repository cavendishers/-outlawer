...
- Architecture V2 blueprint `Phase C: Domain packaging` is now in progress; the first five slices introduced `app.domains.extraction`, `app.domains.replay`, `app.domains.projection`, and `app.domains.retrieval`, moved extraction metadata, extraction payload orchestration, projection persistence, read-side query composition, worker pipeline, replay diff logic, and replay service behavior behind those seams, and kept compatibility shims for legacy imports.
