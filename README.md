...
- Architecture V2 blueprint `Phase C: Domain packaging` is now in progress; the first four slices introduced `app.domains.extraction`, `app.domains.replay`, and `app.domains.projection`, moved extraction metadata, extraction payload orchestration, projection persistence, worker pipeline, replay diff logic, and replay service behavior behind those seams, and kept compatibility shims for legacy imports.
