...
- Architecture V2 blueprint `Phase C: Domain packaging` is now in progress; the first six slices introduced `app.domains.extraction`, `app.domains.replay`, `app.domains.projection`, `app.domains.retrieval`, and `app.domains.governance`, moved extraction metadata, extraction payload orchestration, projection persistence, read-side query composition, worker pipeline, replay diff logic, replay service behavior, merge review, and curation behavior behind those seams, and kept compatibility shims for legacy imports.
