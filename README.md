...
- Architecture V2 blueprint `Phase C: Domain packaging` is now in progress; the first twelve slices introduced `app.domains.extraction`, `app.domains.replay`, `app.domains.projection`, `app.domains.retrieval`, `app.domains.governance`, and `app.domains.operations`, moved extraction metadata, asset-text preparation, local media parsing, OpenRouter extraction helpers, extraction payload orchestration, projection persistence, graph/search/operations read-side composition, worker pipeline, replay diff logic, replay service behavior, merge review, and curation behavior behind those seams, and kept compatibility shims for legacy imports.
