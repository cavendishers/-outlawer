...
- `DONE`: Architecture V2 Phase C is now underway. The first ten domain-packaging slices introduced `app.domains.extraction`, `app.domains.replay`, `app.domains.projection`, `app.domains.retrieval`, `app.domains.governance`, and `app.domains.operations`, moved extraction metadata, asset-text preparation, extraction payload orchestration, projection persistence, graph/search/operations read-side query composition, worker pipeline, replay diff logic, replay service behavior, merge review, and curation behavior behind those packages, and kept compatibility shims so behavior stayed stable during the transition.
