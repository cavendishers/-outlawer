...
- `DONE`: Architecture V2 Phase C is now underway. The first six domain-packaging slices introduced `app.domains.extraction`, `app.domains.replay`, `app.domains.projection`, `app.domains.retrieval`, and `app.domains.governance`, moved extraction metadata, extraction payload orchestration, projection persistence, read-side query composition, worker pipeline, replay diff logic, replay service behavior, merge review, and curation behavior behind those packages, and kept compatibility shims so behavior stayed stable during the transition.
...
1. `MEDIUM`: Domain packaging is partially implemented. Extraction, replay, projection, retrieval, and governance now have real `app.domains.*` implementations, but knowledge, operations, and several supporting graph services are still only partly migrated out of the horizontal `services` layer.
...
- choose the next backend seam from knowledge, operations, or graph-supporting services instead of adding more horizontal helpers
