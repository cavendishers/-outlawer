...
def merge_style_payload(
    ...
    if isinstance(ai_style_payload.get("character_cards"), list):
        for entity in ai_style_payload["character_cards"]:
            ...
            character_cards.append(
                {
                    "entity_temp_id": entity_temp_id,
                    "display_name": safe_string(entity.get("display_name"), entity_name or "未命名人物"),
            ...
            event_narrative.append(
                {
                    ...
                    "body": safe_string(item.get("body"), event["summary"]),
                }
            )
    ...
    return {
        "theme": "chunibyo",
        "title": safe_string(ai_style_payload.get("title"), base_style_payload["title"]),
        "character_cards": character_cards,
        "event_narrative": event_narrative,
    }


def rebuild_style_payload(base_style_payload: dict[str, Any], entities: list[dict[str, Any]], event: dict[str, Any]) -> dict[str, Any]:
    return {
        "theme": "chunibyo",
        "title": base_style_payload["title"],
        "character_cards": [
            {
                "entity_temp_id": entity["temp_id"],
                "display_name": entity["canonical_name"],
        ...
        "event_narrative": [
            {
                ...
                "body": safe_string(event["summary"], "命运的齿轮再次转动。"),
            }
        ],
    }


def merge_similarity_hints(ai_hints: Any) -> list[dict[str, Any]]:
        ...
        merged_hints.append(
            {
                ...
                "target_id": target_id,
                "reason": safe_string(item.get("reason"), "Potential semantic overlap."),
                "confidence": safe_float(item.get("confidence"), 0.5),
            }
        )
    return merged_hints[:5]


def merge_locations(ai_locations: Any) -> list[dict[str, Any]]:
    if not isinstance(ai_locations, list):
        return []
    locations = []
...
def merge_evidence_list(ai_evidence: Any, fallback_text: str) -> list[dict[str, Any]]:
    ...
    for item in ai_evidence:
        if not isinstance(item, dict):
            continue
        text_value = safe_string(item.get("text"), fallback_text)
        evidence_items.append(
            {
                "text": text_value,
                "start": safe_int_or_none(item.get("start")),
                "end": safe_int_or_none(item.get("end")),
            }
        )
    return evidence_items[:3] or [{"text": fallback_text, "start": 0, "end": len(fallback_text)}]
