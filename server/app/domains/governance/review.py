...
def merge_entities(db: Session, *, user_id: str, survivor: Entity, merged: Entity, note: str | None = None) -> None:
    ...
    reassign_event_entities_for_entity_merge(db, survivor_entity_id=survivor.id, merged_entity_id=merged.id)
    db.execute(
    ...
    for alias_row in list_entity_alias_rows(db, merged.id):
        db.delete(alias_row)
...
def merge_events(db: Session, *, user_id: str, survivor: Event, merged: Event, note: str | None = None) -> None:
    ...
    reassign_event_entities_for_event_merge(db, survivor_event_id=survivor.id, merged_event_id=merged.id)
...
def reassign_event_entities_for_entity_merge(db: Session, *, survivor_entity_id: str, merged_entity_id: str) -> None:
    survivor_rows = db.scalars(
        select(EventEntity).where(EventEntity.entity_id == survivor_entity_id).order_by(EventEntity.created_at.asc())
    ).all()
    merged_rows = db.scalars(
        select(EventEntity).where(EventEntity.entity_id == merged_entity_id).order_by(EventEntity.created_at.asc())
    ).all()
    rows_by_event_id = {row.event_id: row for row in survivor_rows}
    for row in merged_rows:
        existing = rows_by_event_id.get(row.event_id)
        if existing:
            merge_event_entity_row(existing, row)
            db.add(existing)
            db.delete(row)
            continue
        row.entity_id = survivor_entity_id
        db.add(row)
...
def reassign_event_entities_for_event_merge(db: Session, *, survivor_event_id: str, merged_event_id: str) -> None:
    survivor_rows = db.scalars(
        select(EventEntity).where(EventEntity.event_id == survivor_event_id).order_by(EventEntity.created_at.asc())
    ).all()
    merged_rows = db.scalars(
        select(EventEntity).where(EventEntity.event_id == merged_event_id).order_by(EventEntity.created_at.asc())
    ).all()
    rows_by_entity_id = {row.entity_id: row for row in survivor_rows}
    for row in merged_rows:
        existing = rows_by_entity_id.get(row.entity_id)
        if existing:
            merge_event_entity_row(existing, row)
            db.add(existing)
            db.delete(row)
            continue
        row.event_id = survivor_event_id
        db.add(row)
...
def merge_event_entity_row(target: EventEntity, source: EventEntity) -> None:
    target.role = target.role or source.role
    if target.relation_type == "participates_in" and source.relation_type != "participates_in":
        target.relation_type = source.relation_type
    target.display_order = min(target.display_order, source.display_order)
    target.confidence_score = max_nullable(target.confidence_score, source.confidence_score)
