...
def list_note_search_results(db: Session, *, user_id: str, q: str, limit: int = 12) -> list[dict]:
    return strip_scores(search_notes(db, user_id, q, limit=limit))


def build_unified_search_payload(
    db: Session,
    *,
    user_id: str,
    q: str | None = None,
    ...
    seed_note = get_owned_note_for_search(db, user_id=user_id, note_id=seed_note_id) if seed_note_id else None

    scored_notes = search_notes(db, user_id, normalized_query, limit=normalized_limit) if normalized_query else []
    scored_entities = search_entities(db, user_id, normalized_query, limit=normalized_limit) if normalized_query else []
    scored_events = search_events(db, user_id, normalized_query, limit=normalized_limit) if normalized_query else []
    similar_notes = similar_note_items(db, user_id, seed_note.id, limit=normalized_limit) if seed_note else []
...
def list_similar_note_results(db: Session, *, user_id: str, note_id: str, limit: int = 5) -> list[dict]:
    note = get_owned_note_for_search(db, user_id=user_id, note_id=note_id)
    return similar_note_items(db, user_id, note.id, limit=limit)
...
def get_owned_note_for_search(db: Session, *, user_id: str, note_id: str) -> Note:
